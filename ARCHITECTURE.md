...
The `packages/intelligence` package currently defines the reasoning domain foundation (`ReasoningRequest`, `ReasoningContext`, `ReasoningResult` with citations and provider metadata), `ContextAssembler` for deterministic context assembly, provider-agnostic `PromptBuilder` / `Prompt` construction, application ports (`KnowledgeRetriever`, `ReasoningProvider`), the `Reason` command for retrieve → assemble → prompt → reason orchestration, and a deterministic `FakeReasoningProvider` for tests. LLM integrations, chat, and agents are not implemented yet.
