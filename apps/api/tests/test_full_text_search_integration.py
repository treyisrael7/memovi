...
def _wait_for_job_status(
    ...
    *,
    ...
    pytest.fail(
        ...
        f"{expected_status.value} within {timeout_seconds}s "
        f"(last status: {current_status})."
    )


def _wait_for_search_documents(
    engine: Engine, *, minimum_count: int = 1, timeout_seconds: float = 5.0
    ...
    pytest.fail(
        f"Expected at least {minimum_count} search documents "
        f"within {timeout_seconds}s (found {count})."
    )
