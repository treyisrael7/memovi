...
def _wait_for_job_status(engine: Engine, processing_job_id: str, expected_status: ProcessingStatus, *, timeout_seconds: float=5.0) -> ProcessingJobRecord:
    ...
    pytest.fail(f'Processing job {processing_job_id} did not reach {expected_status.value} within {timeout_seconds}s (last status: {current_status}).')

def _wait_for_search_documents(engine: Engine, *, minimum_count: int=1, timeout_seconds: float=5.0) -> list[SearchDocumentRecord]:
    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        with Session(engine) as session:
    ...
    pytest.fail(f'Expected at least {minimum_count} search documents within {timeout_seconds}s (found {count}).')
...
def test_upload_process_and_search_returns_memovi_document(full_text_search_client: tuple[TestClient, Engine, InProcessEventDispatcher]) -> None:
    client, engine, _ = full_text_search_client
    response = client.post('/documents', files={'file': ('memovi-overview.md', b'# Memovi\r\n\r\nMemovi is a self-hosted knowledge platform.', 'text/markdown')})
    ...
    with Session(engine) as session:
        ...
        results = search.execute(RetrieveKnowledgeQuery(query='Memovi', mode=RetrievalMode.KEYWORD, limit=10, offset=0, workspace_id=WorkspaceId.default()))
        assert len(results) == 1
        assert results[0].document_id == payload['document_id']
        assert 'Memovi' in results[0].searchable_text
        assert results[0].relevance_score > 0
        missing_results = search.execute(RetrieveKnowledgeQuery(query='missing-term', mode=RetrievalMode.KEYWORD, limit=10, offset=0, workspace_id=WorkspaceId.default()))
