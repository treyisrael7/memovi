...
def search_integration_client() -> Iterator[tuple[TestClient, Engine, InProcessEventDispatcher]]:
    ...
    configure_document_processing(app, session_factory=worker_session_factory, queue=queue, worker_config=DocumentProcessingWorkerConfig(max_retries=3, poll_interval_seconds=0.05), object_storage=object_storage)
    ...
    app.dependency_overrides[get_documents_database_session] = build_documents_database_session(database_session)
    app.dependency_overrides[get_object_storage] = lambda: object_storage
    with TestClient(app, base_url='https://testserver') as client:
        yield (client, engine, dispatcher)
    engine.dispose()

def test_knowledge_materialized_indexes_search_and_publishes_event(search_integration_client: tuple[TestClient, Engine, InProcessEventDispatcher]) -> None:
    ...
    with Session(engine) as session:
        ...
        session.add(KnowledgeItemRecord(id=knowledge_item_id, workspace_id=workspace_id, document_id=document_id, document_version_id=document_version_id, source_type='upload', mime_type='text/markdown', created_at=timestamp, updated_at=timestamp))
        session.add(ChunkRecord(id=str(uuid.uuid4()), workspace_id=workspace_id, knowledge_item_id=knowledge_item_id, document_id=document_id, document_version_id=document_version_id, chunk_index=0, text='Alpha passage.', created_at=timestamp))
        session.add(ChunkRecord(id=str(uuid.uuid4()), workspace_id=workspace_id, knowledge_item_id=knowledge_item_id, document_id=document_id, document_version_id=document_version_id, chunk_index=1, text='Beta passage.', created_at=timestamp))
        session.commit()
    dispatcher.publish(KnowledgeMaterialized(knowledge_item_id=knowledge_item_id, document_id=document_id, document_version_id=document_version_id, chunk_count=2, occurred_at=timestamp, workspace_id=WorkspaceId.default()))
    with Session(engine) as session:
    ...
    indexed_events = [event for event in dispatcher.published_events if isinstance(event, SearchIndexed)]
...
def test_knowledge_materialized_with_missing_knowledge_skips_search(search_integration_client: tuple[TestClient, Engine, InProcessEventDispatcher]) -> None:
    _, engine, dispatcher = search_integration_client
    dispatcher.publish(KnowledgeMaterialized(knowledge_item_id=str(uuid.uuid4()), document_id=str(uuid.uuid4()), document_version_id=str(uuid.uuid4()), chunk_count=0, occurred_at=datetime.now(UTC), workspace_id=WorkspaceId.default()))
    with Session(engine) as session:
...
def test_knowledge_materialized_with_empty_searchable_text_skips_search_indexed(search_integration_client: tuple[TestClient, Engine, InProcessEventDispatcher]) -> None:
    _, engine, dispatcher = search_integration_client
    knowledge_item_id = str(uuid.uuid4())
    document_id = str(uuid.uuid4())
    document_version_id = str(uuid.uuid4())
    ...
    with Session(engine) as session:
        workspace_id = WorkspaceId.default().value
        session.add(KnowledgeItemRecord(id=knowledge_item_id, workspace_id=workspace_id, document_id=document_id, document_version_id=document_version_id, source_type='upload', mime_type='text/markdown', created_at=timestamp, updated_at=timestamp))
        session.add(ChunkRecord(id=str(uuid.uuid4()), workspace_id=workspace_id, knowledge_item_id=knowledge_item_id, document_id=document_id, document_version_id=document_version_id, chunk_index=0, text='   ', created_at=timestamp))
        session.commit()
    dispatcher.publish(KnowledgeMaterialized(knowledge_item_id=knowledge_item_id, document_id=document_id, document_version_id=document_version_id, chunk_count=1, occurred_at=timestamp, workspace_id=WorkspaceId.default()))
    with Session(engine) as session:
        assert session.scalars(select(SearchDocumentRecord)).all() == []
    assert not any((isinstance(event, SearchIndexed) for event in dispatcher.published_events))
