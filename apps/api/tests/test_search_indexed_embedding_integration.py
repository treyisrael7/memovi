...
from memovi_search.infrastructure.persistence.models import SearchDocumentRecord, SearchEmbeddingRecord
...
def test_search_indexed_generates_persists_and_publishes_embedding(embedding_integration_client: tuple[TestClient, Engine, InProcessEventDispatcher]) -> None:
    ...
    generated_events = [event for event in dispatcher.published_events if isinstance(event, EmbeddingGenerated)]
