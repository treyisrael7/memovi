...
def test_2_first_message() -> None:
    ...
    stored = repository.get(ConversationId(conversation_id))
...
def test_3_follow_up() -> None:
    ...
    # History grows to four turns after the follow-up.
    stored = repository.get(ConversationId(conversation_id))
    assert stored is not None
    ...
    # Previous exchange is in the reasoning context / prompt history section.
    ...
    # Current request stays separate from conversation history.
