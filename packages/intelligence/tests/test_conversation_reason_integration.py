...
def test_conversation_reason_updates_conversation_history() -> None:
    knowledge = (RetrievedKnowledge(chunk_id='chunk-memovi', document_id='doc-memovi', text='Memovi is a self-hosted knowledge platform.', score=0.95, document_title='Memovi'),)
    ...
    conversations.append_user_turn(conversation.id, 'Introduce yourself briefly.', workspace_id=WorkspaceId.default())
    conversations.append_assistant_turn(conversation.id, "I am Memovi's reasoning assistant.", workspace_id=WorkspaceId.default())
    prior_history = conversations.load_history(conversation.id, workspace_id=WorkspaceId.default())
    follow_up = 'What is Memovi?'
    result = reason.execute(ReasoningRequest.create(query=follow_up), conversation_history=prior_history)
    conversations.append_user_turn(conversation.id, follow_up, workspace_id=WorkspaceId.default())
    updated = conversations.append_assistant_turn(conversation.id, result.answer, citations=result.citations, workspace_id=WorkspaceId.default())
    ...
    assert [turn.content for turn in result.context.conversation_history.turns] == ['Introduce yourself briefly.', "I am Memovi's reasoning assistant."]
    ...
    assert 'Memovi is a self-hosted knowledge platform.' in prompt.section('retrieved_knowledge').content
