...
def test_conversation_reason_updates_conversation_history() -> None:
    ...
    conversations.append_user_turn(conversation.id, "Introduce yourself briefly.")
    conversations.append_assistant_turn(
        conversation.id,
        "I am Memovi's reasoning assistant.",
    )
    prior_history = conversations.load_history(conversation.id)

    follow_up = "What is Memovi?"
    result = reason.execute(
        ReasoningRequest.create(query=follow_up),
        conversation_history=prior_history,
    )

    conversations.append_user_turn(conversation.id, follow_up)
