...
def _build_send_message() -> tuple[
    ...
    return (
        SendConversationMessage(conversations=conversations, reason=reason),
        conversations,
        repository,
    )
...
def test_send_conversation_message_updates_repository_and_history() -> None:
    ...
    stored = repository.get(conversation.id)
    ...
    updated = repository.get(conversation.id)
    ...
    history = conversations.load_history(conversation.id)
