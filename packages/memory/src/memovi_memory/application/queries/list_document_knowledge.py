...
class ListDocumentKnowledge:
    """Returns canonical knowledge materialized from a document."""
    ...
    def execute(self, document_id: str, *, workspace_id: WorkspaceId) -> list[KnowledgeDto]:
        return [
            self._to_dto(knowledge_item)
            for knowledge_item in self._knowledge_repository.list_by_document(
