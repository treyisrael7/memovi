...
class KnowledgeItemRecord(Base):
    ...
    chunks: Mapped[list[ChunkRecord]] = relationship(back_populates="knowledge_item")
...
class ChunkRecord(Base):
    ...
    knowledge_item_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey("memory_knowledge_items.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    ...
    knowledge_item: Mapped[KnowledgeItemRecord] = relationship(back_populates="chunks")

    __table_args__ = (
        UniqueConstraint(
            "knowledge_item_id",
            "chunk_index",
            name="uq_memory_chunks_knowledge_item_index",
        ),
    )
