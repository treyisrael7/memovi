...
def test_knowledge_item_dto_maps_domain_entity() -> None:
    knowledge_item = KnowledgeItem(id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, created_at=TIMESTAMP, updated_at=TIMESTAMP, workspace_id=WorkspaceId.default())
...
def test_chunk_dto_maps_domain_entity() -> None:
    chunk = Chunk.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(1), text='  Canonical chunk text. ', knowledge_item_id=KNOWLEDGE_ITEM_ID, now=TIMESTAMP, workspace_id=WorkspaceId.default())
...
def test_knowledge_dto_maps_item_and_chunks_in_index_order() -> None:
    knowledge_item = KnowledgeItem(id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, created_at=TIMESTAMP, updated_at=TIMESTAMP, workspace_id=WorkspaceId.default())
    second_chunk = Chunk.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(1), text='Second chunk.', knowledge_item_id=KNOWLEDGE_ITEM_ID, now=TIMESTAMP, workspace_id=WorkspaceId.default())
    first_chunk = Chunk.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(0), text='First chunk.', knowledge_item_id=KNOWLEDGE_ITEM_ID, now=TIMESTAMP, workspace_id=WorkspaceId.default())
    dto = KnowledgeDto.from_knowledge_item_and_chunks(knowledge_item, [second_chunk, first_chunk])
    assert dto.id == KNOWLEDGE_ITEM_ID.value
    assert dto.document_id == DOCUMENT_ID
    assert dto.document_version_id == DOCUMENT_VERSION_ID
    assert dto.created_at == TIMESTAMP
