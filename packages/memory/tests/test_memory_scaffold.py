...
from memovi_memory.infrastructure.persistence.models import Base, ChunkRecord, KnowledgeItemRecord
...
def test_dtos_map_from_domain_entities() -> None:
    knowledge_item = KnowledgeItem.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, workspace_id=WorkspaceId.default())
    chunk = Chunk.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(1), text='Another passage.', knowledge_item_id=knowledge_item.id, workspace_id=WorkspaceId.default())
