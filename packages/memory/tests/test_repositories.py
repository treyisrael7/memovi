...
from memovi_memory.infrastructure.repositories import SqlAlchemyChunkRepository, SqlAlchemyKnowledgeRepository
...
def test_knowledge_repository_round_trips_save_get_list_and_delete() -> None:
    ...
    knowledge_item = KnowledgeItem(id=KnowledgeItemId.new(), document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, created_at=timestamp, updated_at=timestamp, workspace_id=WorkspaceId.default())
...
def test_knowledge_repository_lists_all_and_by_document() -> None:
    ...
    first_item = KnowledgeItem(id=KnowledgeItemId.new(), document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, created_at=earlier, updated_at=earlier, workspace_id=WorkspaceId.default())
    second_item = KnowledgeItem(id=KnowledgeItemId.new(), document_id=DOCUMENT_ID, document_version_id=OTHER_DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, created_at=later, updated_at=later, workspace_id=WorkspaceId.default())
    third_item = KnowledgeItem(id=KnowledgeItemId.new(), document_id=OTHER_DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, created_at=later, updated_at=later, workspace_id=WorkspaceId.default())
    with session_factory() as session:
        repository = SqlAlchemyKnowledgeRepository(session)
...
def test_knowledge_repository_hides_items_from_other_workspaces() -> None:
    ...
    with session_factory() as session:
        ...
        assert [item.id for item in repository.list_by_workspace(workspace_id=WorkspaceId.default())] == [
            owned.id
        ]
...
def test_chunk_repository_round_trips_save_many_list_and_delete() -> None:
    ...
    first = Chunk.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(0), text=' First chunk. ', knowledge_item_id=knowledge_item_id, workspace_id=WorkspaceId.default())
    second = Chunk.create(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(1), text='Second chunk.', knowledge_item_id=knowledge_item_id, workspace_id=WorkspaceId.default())
    with session_factory() as session:
    ...
    with session_factory() as session:
        repository = SqlAlchemyChunkRepository(session)
        loaded = repository.list_by_document_version(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID)
        assert len(loaded) == 2
        assert loaded[0].text == 'First chunk.'
        assert loaded[0].knowledge_item_id == knowledge_item_id
        assert loaded[1].chunk_index.value == 1
    updated_second = Chunk(id=second.id, knowledge_item_id=knowledge_item_id, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, chunk_index=ChunkIndex(1), text='Updated second chunk.', created_at=second.created_at, workspace_id=WorkspaceId.default())
    with session_factory() as session:
        repository = SqlAlchemyChunkRepository(session)
    ...
    with session_factory() as session:
        ...
        repository.delete_by_document_version(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID)
        session.commit()
    with session_factory() as session:
        repository = SqlAlchemyChunkRepository(session)
        assert repository.list_by_document_version(document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID) == []
    engine.dispose()
