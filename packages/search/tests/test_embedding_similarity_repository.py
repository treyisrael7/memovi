...
# Unit vectors chosen so cosine similarity is exact and easy to assert.
...
def test_similarity_search_returns_nearest_neighbors_ordered_by_cosine_similarity() -> None:
    ...
    near_document = _build_search_document(
        knowledge_item_id=KNOWLEDGE_NEAR,
        searchable_text="Near neighbor document.",
    )
    mid_document = _build_search_document(
        knowledge_item_id=KNOWLEDGE_MID,
        searchable_text="Mid neighbor document.",
    )
    far_document = _build_search_document(
        knowledge_item_id=KNOWLEDGE_FAR,
        searchable_text="Far neighbor document.",
    )
    ...
    with session_factory() as session:
        ...
        embedding_repository.save(
            _build_embedding(search_document_id=near_document.id, vector=NEAR_VECTOR),
        )
        embedding_repository.save(
            _build_embedding(search_document_id=mid_document.id, vector=MID_VECTOR),
        )
        embedding_repository.save(
            _build_embedding(search_document_id=far_document.id, vector=FAR_VECTOR),
        )
        session.commit()
    ...
    with session_factory() as session:
        embedding_repository = SqlAlchemyEmbeddingRepository(session)
        results = embedding_repository.similarity_search(query, limit=3)

        assert [result.search_document.knowledge_item_id for result in results] == [
            KNOWLEDGE_NEAR,
            KNOWLEDGE_MID,
            KNOWLEDGE_FAR,
        ]
        ...
        assert results[1].relevance_score == pytest.approx(0.0, abs=1e-6)
        assert results[2].relevance_score == pytest.approx(-1.0, abs=1e-6)
...
def test_similarity_search_respects_limit() -> None:
    ...
    with session_factory() as session:
        embedding_repository = SqlAlchemyEmbeddingRepository(session)
        results = embedding_repository.similarity_search(query, limit=1)
