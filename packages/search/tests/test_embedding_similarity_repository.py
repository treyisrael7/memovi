...
def test_similarity_search_returns_nearest_neighbors_ordered_by_cosine_similarity() -> None:
    ...
    near_document = _build_search_document(
        knowledge_item_id=KNOWLEDGE_NEAR, searchable_text="Near neighbor document."
    )
    mid_document = _build_search_document(
        knowledge_item_id=KNOWLEDGE_MID, searchable_text="Mid neighbor document."
    )
    far_document = _build_search_document(
        knowledge_item_id=KNOWLEDGE_FAR, searchable_text="Far neighbor document."
    )
    ...
    with session_factory() as session:
        embedding_repository = SqlAlchemyEmbeddingRepository(session)
        results = embedding_repository.similarity_search(
            query, limit=3, workspace_id=WorkspaceId.default()
        )
...
def test_similarity_search_respects_limit() -> None:
    ...
    with session_factory() as session:
        embedding_repository = SqlAlchemyEmbeddingRepository(session)
        results = embedding_repository.similarity_search(
            query, limit=1, workspace_id=WorkspaceId.default()
        )
