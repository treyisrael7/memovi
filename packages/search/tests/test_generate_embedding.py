...
def _search_document() -> SearchDocument:
    return SearchDocument(id=SearchDocumentId.new(), knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, searchable_text=SEARCHABLE_TEXT, created_at=datetime(2026, 7, 10, 12, 0, tzinfo=UTC), updated_at=datetime(2026, 7, 10, 12, 0, tzinfo=UTC), workspace_id=WorkspaceId.default())

def _build_use_case(*, documents: list[SearchDocument] | None=None) -> tuple[GenerateEmbedding, FakeSearchRepository, FakeEmbeddingRepository, FakeEmbeddingProvider, FakeEventPublisher]:
    provider = FakeEmbeddingProvider()
    search_repository = FakeSearchRepository(documents)
    embedding_repository = FakeEmbeddingRepository()
    event_publisher = FakeEventPublisher()
    use_case = GenerateEmbedding(search_repository=search_repository, embedding_repository=embedding_repository, embedding_generation_service=EmbeddingGenerationService(provider=provider), event_publisher=event_publisher)
...
def test_generate_embedding_loads_document_generates_persists_and_publishes() -> None:
    ...
    use_case, _, embedding_repository, provider, event_publisher = _build_use_case(documents=[search_document])
...
def test_generate_embedding_updates_existing_projection_for_same_provider_model() -> None:
    search_document = _search_document()
    use_case, _, embedding_repository, provider, _event_publisher = _build_use_case(documents=[search_document])
    existing = Embedding.create(search_document_id=search_document.id, provider=provider.provider, model=provider.model, vector=[0.0, 0.0, 0.0, 0.0])
...
def test_generate_embedding_replaces_projection_when_provider_changes() -> None:
    search_document = _search_document()
    use_case, _, embedding_repository, provider, _event_publisher = _build_use_case(documents=[search_document])
    existing = Embedding.create(search_document_id=search_document.id, provider='openai', model='text-embedding-3-small', vector=[0.1, 0.2])
    embedding_repository.save(existing)
    result = use_case.execute(GenerateEmbeddingCommand(search_document_id=search_document.id.value))
...
def test_generate_embedding_raises_when_search_document_missing() -> None:
    ...
    with pytest.raises(SearchDocumentNotFoundError):
        use_case.execute(GenerateEmbeddingCommand(search_document_id=str(SearchDocumentId.new())))
