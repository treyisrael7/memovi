...
def test_keyword_retriever_maps_repository_hits() -> None:
    ...
    repository.results = [
        RankedSearchDocument(search_document=document, relevance_score=0.42),
    ]
    retriever = KeywordRetriever(search_repository=repository)

    results = retriever.retrieve(RetrievalRequest(query="  Memovi  ", limit=10))
...
def test_keyword_retriever_returns_empty_for_blank_query() -> None:
    ...
    assert retriever.retrieve(RetrievalRequest(query="   ", limit=10)) == []
