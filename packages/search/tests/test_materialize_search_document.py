...
import pytest
from memovi_search.application.commands import MaterializeSearchDocument, MaterializeSearchDocumentCommand
...
class FakeSearchRepository(SearchRepository):
    ...
    def delete_document(self, search_document_id: SearchDocumentId) -> None:
        self.saved_documents = [document for document in self.saved_documents if document.id != search_document_id]
...
class SpySearchMaterializer:
    ...
    def materialize(
        ...
        *,
        ...
        self.materialize_calls.append((knowledge_item_id, document_id, document_version_id, len(chunk_texts)))
...
def build_use_case() -> tuple[MaterializeSearchDocument, SpySearchMaterializer, FakeSearchRepository, list[str]]:
    operations: list[str] = []
    search_materializer = SpySearchMaterializer(SearchMaterializer())
    search_repository = FakeSearchRepository(operations)
    use_case = MaterializeSearchDocument(search_materializer=cast(SearchMaterializer, search_materializer), search_repository=search_repository)
...
def test_materialize_search_document_orchestrates_materializer_and_repository() -> None:
    ...
    result = use_case.execute(MaterializeSearchDocumentCommand(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, chunk_texts=['Alpha passage.', 'Beta passage.'], workspace_id=WorkspaceId.default()))
    assert search_materializer.materialize_calls == [(KNOWLEDGE_ITEM_ID, DOCUMENT_ID, DOCUMENT_VERSION_ID, 2)]
...
def test_materialize_search_document_persists_exactly_once() -> None:
    ...
    use_case.execute(MaterializeSearchDocumentCommand(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, chunk_texts=['Single chunk text.'], workspace_id=WorkspaceId.default()))
...
def test_materialize_search_document_skips_persistence_on_materialization_failure() -> None:
    ...
    with pytest.raises(InvalidSearchMaterializationError):
        use_case.execute(MaterializeSearchDocumentCommand(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, chunk_texts=['   '], workspace_id=WorkspaceId.default()))
    assert search_materializer.materialize_calls == [(KNOWLEDGE_ITEM_ID, DOCUMENT_ID, DOCUMENT_VERSION_ID, 1)]
    assert operations == []
