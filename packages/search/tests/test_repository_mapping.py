...
from memovi_search.infrastructure.repositories.sqlalchemy_embedding_repository import SqlAlchemyEmbeddingRepository
from memovi_search.infrastructure.repositories.sqlalchemy_search_repository import SqlAlchemySearchRepository
...
def test_document_mapping_round_trips_between_domain_and_record() -> None:
    ...
    search_document = SearchDocument(id=SearchDocumentId.new(), knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, searchable_text='Retrievable passage.', created_at=timestamp, updated_at=timestamp, workspace_id=WorkspaceId.default())
...
def test_embedding_mapping_round_trips_between_domain_and_record() -> None:
    ...
    embedding = Embedding(id=EmbeddingId.new(), search_document_id=search_document_id, provider='openai', model='text-embedding-3-small', dimensions=4, vector=(0.1, 0.2, 0.3, 0.4))
