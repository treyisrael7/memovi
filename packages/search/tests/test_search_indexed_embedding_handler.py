...
class FakeSearchRepository(SearchRepository):
    ...
    def search(self, query: str, limit: int, offset: int, *, document_id: str | None=None, document_version_id: str | None=None, source_type: str | None=None, mime_type: str | None=None, created_after: datetime | None=None, created_before: datetime | None=None) -> list[RankedSearchDocument]:
        return []
...
class FakeEmbeddingRepository(EmbeddingRepository):
    ...
    def get_by_search_document(self, search_document_id: SearchDocumentId) -> Embedding | None:
    ...
    def similarity_search(self, query_vector: EmbeddingVector, limit: int) -> list[RankedSearchDocument]:
        return []
...
def _search_document() -> SearchDocument:
    return SearchDocument(id=SearchDocumentId.new(), knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, searchable_text=SEARCHABLE_TEXT, created_at=INDEXED_AT, updated_at=INDEXED_AT, workspace_id=WorkspaceId.default())

def _build_handler(*, document: SearchDocument | None) -> tuple[SearchIndexedEmbeddingHandler, FakeEmbeddingRepository, FakeEventPublisher, FakeSessionFactory]:
    ...
    def factory(session: OrmSession, publisher: EventPublisher) -> GenerateEmbedding:
        del session
        return GenerateEmbedding(search_repository=search_repository, embedding_repository=embedding_repository, embedding_generation_service=EmbeddingGenerationService(provider=FakeEmbeddingProvider()), event_publisher=publisher)
    handler = SearchIndexedEmbeddingHandler(generate_embedding_factory=factory, event_publisher=event_publisher, session_factory=cast(Callable[[], OrmSession], session_factory))
...
def test_handler_generates_persists_and_publishes_embedding_generated() -> None:
    ...
    handler, embedding_repository, event_publisher, session_factory = _build_handler(document=search_document)
    handler.handle(SearchIndexed(search_document_id=search_document.id.value, knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, indexed_at=INDEXED_AT))
...
def test_handler_skips_when_search_document_missing() -> None:
    handler, embedding_repository, event_publisher, session_factory = _build_handler(document=None)
    handler.handle(SearchIndexed(search_document_id=str(SearchDocumentId.new()), knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, indexed_at=INDEXED_AT))
