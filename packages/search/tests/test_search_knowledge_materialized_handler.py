...
from memovi_search.application.commands import MaterializeSearchDocument, MaterializeSearchDocumentCommand, MaterializeSearchDocumentResult
from memovi_search.application.dto import KnowledgeChunkReadDto, KnowledgeMaterializedNotification, KnowledgeReadDto
...
class FakeSearchRepository(SearchRepository):
    ...
    def list_documents(self, *, workspace_id: WorkspaceId | None = None) -> list[SearchDocument]:
...
def _build_handler(*, knowledge: KnowledgeReadDto | None, operations: list[str]) -> tuple[SearchKnowledgeMaterializedHandler, FakeKnowledgeReader, SpyMaterializeSearchDocument, FakeSearchRepository, FakeEventPublisher]:
    ...
    materialize = SpyMaterializeSearchDocument(MaterializeSearchDocument(search_materializer=SearchMaterializer(), search_repository=search_repository))
    session_factory = FakeSessionFactory()
    handler = SearchKnowledgeMaterializedHandler(knowledge_reader=knowledge_reader, materialize_search_document_factory=lambda _session: cast(MaterializeSearchDocument, materialize), event_publisher=event_publisher, session_factory=cast(Callable[[], OrmSession], session_factory))
...
def test_handler_materializes_persists_and_publishes_search_indexed() -> None:
    operations: list[str] = []
    handler, knowledge_reader, materialize, search_repository, event_publisher = _build_handler(knowledge=_knowledge_read_dto(chunks=(KnowledgeChunkReadDto(chunk_index=0, text='Alpha.'), KnowledgeChunkReadDto(chunk_index=1, text='Beta.'))), operations=operations)
    handler.handle(KnowledgeMaterializedNotification(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, occurred_at=NOTIFICATION_AT, workspace_id=WorkspaceId.default()))
...
def test_handler_skips_persistence_when_knowledge_is_missing() -> None:
    operations: list[str] = []
    handler, knowledge_reader, materialize, _, event_publisher = _build_handler(knowledge=None, operations=operations)
    handler.handle(KnowledgeMaterializedNotification(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, occurred_at=NOTIFICATION_AT, workspace_id=WorkspaceId.default()))
    assert knowledge_reader.get_knowledge_calls == [KNOWLEDGE_ITEM_ID]
...
def test_handler_skips_search_indexed_when_searchable_text_is_empty() -> None:
    operations: list[str] = []
    handler, _, materialize, search_repository, event_publisher = _build_handler(knowledge=_knowledge_read_dto(chunks=(KnowledgeChunkReadDto(chunk_index=0, text='   '),)), operations=operations)
    handler.handle(KnowledgeMaterializedNotification(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, occurred_at=NOTIFICATION_AT, workspace_id=WorkspaceId.default()))
    assert len(materialize.execute_calls) == 1
    assert operations == []
