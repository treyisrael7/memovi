...
from memovi_search.infrastructure.persistence.models import Base, SearchDocumentRecord, SearchEmbeddingRecord
...
def test_dtos_map_from_domain_entities() -> None:
    search_document = SearchDocument.create(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type=SOURCE_TYPE, mime_type=MIME_TYPE, searchable_text='Retrievable passage.', workspace_id=WorkspaceId.default())
    embedding = Embedding.create(search_document_id=search_document.id, provider='openai', model='text-embedding-3-small', vector=[0.1, 0.2])
