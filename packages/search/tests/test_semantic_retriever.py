...
def test_semantic_retriever_embeds_query_and_maps_hits() -> None:
    document = SearchDocument.create(knowledge_item_id=KNOWLEDGE_ITEM_ID, document_id=DOCUMENT_ID, document_version_id=DOCUMENT_VERSION_ID, source_type='upload', mime_type='text/markdown', searchable_text='Memovi knowledge', workspace_id=WorkspaceId.default())
    ...
    results = retriever.retrieve(RetrievalRequest(query='  Memovi  ', limit=5, workspace_id=WorkspaceId.default()))
...
def test_semantic_retriever_returns_empty_for_blank_query() -> None:
    repository = FakeEmbeddingRepository()
    retriever = SemanticRetriever(embedding_provider=FakeEmbeddingProvider(), embedding_repository=repository)
    assert retriever.retrieve(RetrievalRequest(query=' ', limit=5, workspace_id=WorkspaceId.default())) == []
