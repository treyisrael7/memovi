...
def test_semantic_retriever_embeds_query_and_maps_hits() -> None:
    ...
    repository.results = [
        RankedSearchDocument(search_document=document, relevance_score=0.91),
    ]
    ...
    results = retriever.retrieve(RetrievalRequest(query="  Memovi  ", limit=5))
...
def test_semantic_retriever_returns_empty_for_blank_query() -> None:
    ...
    assert retriever.retrieve(RetrievalRequest(query=" ", limit=5)) == []
