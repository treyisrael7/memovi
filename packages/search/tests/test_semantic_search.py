...
def test_semantic_search_facade_uses_semantic_mode() -> None:
    ...
    results = use_case.execute(
        SemanticSearchQuery(query="  Memovi  ", limit=10, workspace_id=WorkspaceId.default())
    )
    assert results == [
        SearchResultDto(
...
def test_semantic_search_returns_empty_for_blank_query() -> None:
    ...
    use_case = SemanticSearch(
        retrieval_engine=RetrievalEngine(
            keyword_retriever=FakeRetriever(), semantic_retriever=semantic
        )
    )
    assert (
        use_case.execute(
            SemanticSearchQuery(query="   ", limit=10, workspace_id=WorkspaceId.default())
        )
        == []
    )
